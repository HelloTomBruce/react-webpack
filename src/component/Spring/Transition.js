...
const Transition = () => {
    ...
    useEffect(() => void setInterval(() => set(state => (state + 1) % 4), 9000), []);
    return (
        <div className="bg-container">
            {transitions.map(({ item, props, key }) => (
                <animated.div key={key} className="bg" style={{ ...props, backgroundImage: `url(https://images.unsplash.com/${item.url}&auto=format&fit=crop)`, backgroundRepeat: "no-repeat" }} />
            ))}
        </div>
    );
};
